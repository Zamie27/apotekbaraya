<div>
    ...
    {{-- Category Navigation --}}
    <div class="mb-6">
        <div class="relative flex justify-center w-full mx-2">
            <div class="absolute left-0 z-10 top-1/2 -translate-y-1/2 backdrop-blur-sm bg-base-200/60 rounded-full">
                <button
                    ...
                    onclick="scrollKategoriPage(-400)"
            ...
            <div
                ...
                class="flex max-w-full gap-2 px-4 py-2 overflow-x-auto snap-x scroll-smooth scrollbar-hide">
                <a href="/kategori" class="btn btn-sm {{ !$categoryId ? 'btn-success' : 'btn-outline btn-success' }} whitespace-nowrap">
                    Semua Kategori
                </a>
                @foreach($categories as $cat)
                <a href="/kategori/{{ $cat->slug }}" class="btn btn-sm {{ $categoryId == $cat->category_id ? 'btn-success' : 'btn-outline btn-success' }} whitespace-nowrap">
    ...
    <div class="mt-8 flex justify-center">
        <nav class="flex items-center space-x-1" aria-label="Pagination">
            ...
            @else
            <button wire:click="previousPage" class="px-3 py-2 text-gray-600 hover:text-gray-900 transition-colors">
                <svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                    <path fill-rule="evenodd" d="M12.79 5.23a.75.75 0 01-.02 1.06L8.832 10l3.938 3.71a.75.75 0 11-1.04 1.08l-4.5-4.25a.75.75 0 010-1.08l4.5-4.25a.75.75 0 011.06.02z" clip-rule="evenodd" />
                </svg>
            </button>
            ...
            {{-- First Page --}}
            @if($start > 1)
            <button wire:click="gotoPage(1)" class="px-3 py-2 text-sm font-medium text-gray-700 hover:text-gray-900 transition-colors">1</button>
            @if($start > 2)
            ...
            @for($page = $start; $page <= $end; $page++)
                ...
                <span class="px-3 py-2 text-sm font-semibold text-success border-b-2 border-success">{{ $page }}</span>
                @else
                <button wire:click="gotoPage({{ $page }})" class="px-3 py-2 text-sm font-medium text-gray-700 hover:text-gray-900 transition-colors">{{ $page }}</button>
                @endif
                @endfor
                ...
                @if($end < $products->lastPage())
                    @if($end < $products->lastPage() - 1)
                        <span class="px-3 py-2 text-sm text-gray-500">...</span>
                        @endif
                        <button wire:click="gotoPage({{ $products->lastPage() }})" class="px-3 py-2 text-sm font-medium text-gray-700 hover:text-gray-900 transition-colors">{{ $products->lastPage() }}</button>
                        @endif

                        {{-- Next Page Link --}}
                        @if($products->hasMorePages())
                        <button wire:click="nextPage" class="px-3 py-2 text-gray-600 hover:text-gray-900 transition-colors">
                            <svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
